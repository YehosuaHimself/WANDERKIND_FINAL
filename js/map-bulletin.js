// @ts-nocheck
/**
 * /js/map-bulletin.js · The Bulletin · ruled May 2026.
 *
 * The Bulletin is what the Map says today, in this place. It is a drawer
...
document.addEventListener('DOMContentLoaded', () => {
  const handle = $('bulletin-handle');
  const drawer = $('bulletin');
  ...
  const closeBtn = $('bulletin-close');
  if (closeBtn) closeBtn.addEventListener('click', close);

  const pullToRefresh = $('bulletin-refresh');
...
async function open() {
  state.open = true;
  $('bulletin').classList.add('open');
  $('bulletin').setAttribute('aria-hidden', 'false');
  $('bulletin-handle').setAttribute('aria-expanded', 'true');
  await fetchRows(false);
  render();
  // Trap-light focus management: move focus to close button
  const cb = $('bulletin-close');
  if (cb) cb.focus();
}

function close() {
  state.open = false;
  $('bulletin').classList.remove('open');
  $('bulletin').setAttribute('aria-hidden', 'true');
  $('bulletin-handle').setAttribute('aria-expanded', 'false');
  $('bulletin-handle').focus();
}
...
async function fetchRows(force) {
  ...
  try {
    ...
    console.warn('[bulletin] fetch failed', err);
  } finally {
...
function render() {
  const list = $('bulletin-list');
  const empty = $('bulletin-empty');
...
function cardEl(c) {
  ...
  a.className = `bul-card bul-card--${c.kind}`;
  ...
  if (c.kind === 'door') {
    glyph = '⌂';
    body = `<div class="bul-h"><span class="bul-actor">${safe(c.actor_name)}</span> opened a door</div>
            <div class="bul-sub">${safe(c.region) || ''}</div>`;
  } else if (c.kind === 'vouch') {
    glyph = '◐';
    body = `<div class="bul-h">A vouch was published</div>
            <div class="bul-sub bul-quote">"${safe(c.excerpt)}${(c.excerpt && c.excerpt.length >= 80) ? '…' : ''}"</div>`;
  } else if (c.kind === 'stamp') {
    glyph = '◉';
    body = `<div class="bul-h"><span class="bul-actor">${safe(c.actor_name) || 'A walker'}</span> sealed a stamp</div>
            <div class="bul-sub">${safe(c.stamp_name)}${c.region ? ' · ' + safe(c.region) : ''}</div>`;
  } else if (c.kind === 'proposal') {
    glyph = '✚';
    body = `<div class="bul-h"><span class="bul-actor">A stamp</span> was proposed</div>
            <div class="bul-sub">${safe(c.stamp_name)}${c.region ? ' · ' + safe(c.region) : ''}</div>`;
  }

  a.innerHTML = `
    <span class="bul-glyph" aria-hidden="true">${glyph}</span>
    <div class="bul-body">${body}</div>
    <span class="bul-ago">${ago}</span>
  `;
  return a;
}
